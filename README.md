...
그 태그가 달린 글이 모입니다. 현재 보안 실무 글 250편이 올라가 있습니다.
...
색인 상태는 글마다 구글·네이버·빙 배지로 표시됩니다. 배지를 누르면 그 포털의 `site:` 검색이
팝업으로 열립니다 — 누르는 것으로 상태가 바뀌지는 않습니다.

**구글 배지는 자동으로 갱신됩니다.** 배포할 때마다(하루 두 번) [index-status.mts](scripts/index-status.mts)
가 Search Console URL 검사 API 로 글마다 색인 여부를 확인해 Firestore 에 기록합니다. 검색 결과를
긁는 것은 각 사 약관 위반이라, 공식 API 가 유일한 방법입니다. 네이버·빙은 색인 여부를 알려주는
공개 API 가 없어 배지가 자동으로 켜지지 않습니다.

```bash
npx tsx scripts/index-status.mts --dry   # 조회만
npx tsx scripts/index-status.mts         # Firestore 갱신
```

쓰려면 세 가지가 필요합니다.

1. GCP 콘솔에서 **Google Search Console API** 사용 설정
2. Search Console → 설정 → 사용자 및 권한에서 서비스 계정 이메일을 **소유자** 로 추가
   (URL 검사 API 는 소유자만 호출할 수 있습니다)
3. GitHub → Secrets 에 `GCP_SERVICE_ACCOUNT` (서비스 계정 JSON 전체).
   도메인 속성을 쓴다면 Variables 에 `GSC_PROPERTY` = `sc-domain:blog.sanghak.kr`

키가 없으면 이 단계는 건너뛰고 배포는 그대로 진행됩니다.
...
글·태그·댓글·감사 로그·보안 설정은 모두 Firestore 에 있습니다. 게시글 도식 350개는 저장소의
...
그래서 배포 워크플로가 **매일 06:00 과 18:00(KST) 두 번 다시 빌드**합니다. 글을 쓰거나
고치면 늦어도 반나절 안에 검색엔진 쪽에도 반영됩니다. 급하면 Actions 에서 수동 실행하면 됩니다.
