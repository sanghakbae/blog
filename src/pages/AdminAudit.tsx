...
export default function AdminAudit() {
  ...
  return (
    <div>
      ...
      {!entries && <p className="text-sm text-[var(--color-muted)]">불러오는 중…</p>}
      {entries?.length === 0 && !error && (
        <p className="text-sm text-[var(--color-muted)]">아직 기록이 없습니다.</p>
      )}
            ...
            <tbody>
              {entries.map((e) => (
                <tr key={e.id} className="border-b border-[var(--color-line)] align-top">
                  <td className="py-2.5 pr-4 whitespace-nowrap tabular-nums text-xs text-[var(--color-muted)]">
                    {formatAt(e)}
                  </td>
                  <td className="py-2.5 pr-4 whitespace-nowrap">
                    {AUDIT_LABELS[e.action] ?? e.action}
                  </td>
                  <td className="py-2.5 pr-4 text-xs text-[var(--color-muted)]">{e.actorEmail}</td>
                  <td className="max-w-[16rem] truncate py-2.5 pr-4 text-xs text-[var(--color-muted)]">
                    {e.target}
                  </td>
                  <td className="py-2.5 text-xs text-[var(--color-muted)]">{e.detail}</td>
                </tr>
              ))}
            </tbody>
          </table>
